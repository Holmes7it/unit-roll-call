...
// Define search query param validation
...
const empty = (batchCode: string = ""): Omit<Soldier, "id" | "createdAt"> => ({
  serviceNumber: "",
  rank: "Private",
  lastName: "",
  firstName: "",
...
function AddSoldier() {
  ...
  const { batch: urlBatch } = Route.useSearch();
  ...
  const onPhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        if (!ctx) { set("photo", String(reader.result)); return; }
        ctx.drawImage(img, 0, 0, w, h);
        set("photo", canvas.toDataURL("image/jpeg", 0.82));
      };
      img.onerror = () => set("photo", String(reader.result));
            ...
            <CardHeader className="border-b border-border/50 bg-muted/20">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <CardDescription>Upload an official service photograph for the profile.</CardDescription>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
            ...
            <CardContent className="pt-8 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* Deployment Batch field */}
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-tighter">Deployment Batch</Label>
                    {urlBatch ? (
                      <Input
                        disabled
                        value={urlBatch}
                        className="bg-muted border-border/50 font-mono text-xs uppercase cursor-not-allowed opacity-80"
                      />
                    ) : (
                      <Select value={form.batch} onValueChange={(v) => set("batch", v)}>
                        <SelectTrigger className="bg-background/50 border-border/50">
                          <SelectValue placeholder="Select Deployment Batch" />
                        </SelectTrigger>
                        <SelectContent>
